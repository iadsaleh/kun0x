    ...
    setup() {
        ...
        const fetchGames = async () => {
            try {
                ...
                console.log("DEBUG: Fetched games count:", games.value.length);
                console.log("DEBUG: Active games:", games.value.filter(g => g.status === 'active').length);
                console.log("DEBUG: Not Active games:", games.value.filter(g => g.status === 'not_active').length);
            } catch (error) {
        ...
        const fetchPlans = async () => {
            try {
                const response = await fetch(`plans.json?t=${Date.now()}`);
